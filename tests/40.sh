...
echo "-- Building Redis 4.0 image"
docker build -t redis-4.0 ../4.0/
docker network create redis_test_net

echo
echo "-- Testing Redis 4.0 is running"

docker run --name base_1 -d --net redis_test_net -e 'REDIS_PASSWORD=pass' redis-4.0; sleep 5
docker run --name base_2 -d --net redis_test_net redis-4.0; sleep 5
...
echo
echo "-- Testing Cluster Redis 4.0"
echo

echo "-- Create node1"
docker run --name node1 -d --net redis_test_net -e 'REDIS_CLUSTER_ENABLED=true' redis-4.0 --appendonly yes; sleep 5
echo
echo "-- Create node2"
docker run --name node2 -d --net redis_test_net -e 'REDIS_CLUSTER_ENABLED=true' redis-4.0 --appendonly yes; sleep 5
echo
echo "-- Create node3"
docker run --name node3 -d --net redis_test_net -e 'REDIS_CLUSTER_ENABLED=true' redis-4.0 --appendonly yes; sleep 5
echo
echo "-- Create node4"
docker run --name node4 -d --net redis_test_net -e 'REDIS_CLUSTER_ENABLED=true' redis-4.0 --appendonly yes; sleep 5
echo
echo "-- Create node5"
docker run --name node5 -d --net redis_test_net -e 'REDIS_CLUSTER_ENABLED=true' redis-4.0 --appendonly yes; sleep 5
echo
echo "-- Create node6 as provider"
docker run --name node6 -d --net redis_test_net -e 'REDIS_CLUSTER_ENABLED=true' redis-4.0 --appendonly yes; sleep 5
